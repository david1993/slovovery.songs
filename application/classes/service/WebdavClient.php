...
class webdav_client
{
    ...
    function getInRar($path, &$buffer)
    {
        $this->_path = $this->_translate_uri($path);
        $this->_header_unset();
        $this->_create_basic_request('GET');
        $this->_header_add('TE: gzip');
    ...
    function get_file_inrar($srcpath, $localpath)
    {

        if ($this->getInRar($srcpath, $buffer)) {
            $handle = fopen($localpath, 'w');
            if ($handle) {
                fwrite($handle, $buffer);
                fclose($handle);
    ...
    function ls($path)
    {
        ...
        $this->_header_add('Accept: */*');
        $this->_header_add('Depth: 1');
        $this->_send_request();
        $this->_get_respond();// устанавливаем переменные хедер и боди
        $response = $this->_process_respond();
        //print_r($response);
        // validate the response ... (only basic validation)
        // check http-version
    ...
    function _create_basic_request($method)
    {
        ...
        $this->_header_add(sprintf('Host: %s', $this->_site));
    ...
    function _get_respond($j)
    {
        ...
        switch (true) {
            case (preg_match('/Transfer\\-Encoding:\\s+chunked\\r\\n/', $header)):
                /*$handle = fopen('5.txt', 'w+'); // truncate + attempt to create

                              while($data=fread($this->_fp, 100000000)){
                                  fwrite($handle, $data);
                                  unset($data);
                              }
                              fclose($handle);
            ...
            case preg_match('/Content\\-Length:\\s+([0-9]*)\\r\\n/', $header, $matches):
                ...
                if ($matches[1] <= $max_chunk_size) {
                    ...
                    if ($matches[1] > 0) {
                        //$this->_error_log('$matches[1]'.$matches[1]);
                        //	$buffer = fread($this->_fp,5);$buffer .= fread($this->_fp,1);$buffer .= fread($this->_fp,1);
                        $buffer = '';
                        $tmp = fgets($this->_fp, (int)$matches[1]);
                        $buffer .= $tmp;
                        //echo $tmp;
                    ...
                    do {
                        ...
                        //echo $mod.'<br>';
                        if ($mod == $max_chunk_size) {
                            $chunk_size = $max_chunk_size;
                        } else {
                            $chunk_size = $matches[1] - strlen($buffer);
                            $m++;
                            //echo 'm++';echo $chunk_size.' ';
                        }
                        $tmp = fread($this->_fp, $chunk_size);
                        $buffer .= $tmp;
                        //	echo $tmp;
                        //	echo strlen($buffer).'<br>';
                        $this->_error_log('mod: ' . $mod . ' chunk: ' . $chunk_size . ' total: ' . strlen($buffer));
                    } while ($mod == $max_chunk_size || $m < 20);
                }
            ...
            default:
                ...
                //echo fgets($this->_fp, 8094);exit;
                while ($tmp = fgets($this->_fp, 4048) != '') {
                    //	$tmp=fgets($this->_fp, 4048);
                    print_r($tmp);
                    $buffer .= $tmp;
